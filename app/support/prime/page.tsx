...
function PrimeContent() {
  return (
    <SupportLayout title="Prime Tailors Management" subtitle="Panel 3 — Manage Prime badges & benefits">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h3 className="font-semibold text-sm text-gray-800">Prime Tailors List</h3>
          <button className="bg-yellow-500 text-white text-sm px-4 py-2 rounded-lg hover:bg-yellow-600">⭐ Assign Prime Badge</button>
        </div>
        <table className="w-full">
          <thead>
            <tr className="text-[11px] text-gray-400 uppercase tracking-wider bg-gray-50 border-b border-gray-100">
              ...
              <th className="text-left px-4 py-3">Earnings</th><th className="text-left px-4 py-3">Badge</th>
              <th className="text-left px-4 py-3">Since</th><th className="text-left px-4 py-3">Actions</th>
            </tr>
          </thead>
          <tbody>
            {primeTailors.map((t) => (
              <tr key={t.id} className="border-t border-gray-50 hover:bg-gray-50">
                ...
                <td className="px-4 py-4 text-sm font-semibold">★ {t.rating}</td>
                <td className="px-4 py-4 text-sm font-semibold text-green-600">{t.earnings}</td>
                <td className="px-4 py-4"><span className={`text-xs px-2.5 py-1 rounded-full font-semibold border ${t.badge==="Gold Prime"?"bg-yellow-50 text-yellow-700 border-yellow-200":"bg-gray-100 text-gray-600 border-gray-200"}`}>{t.badge}</span></td>
                ...
                <td className="px-4 py-4">
                  <div className="flex gap-2">
                    ...
                    <button className="text-xs bg-yellow-50 border border-yellow-200 text-yellow-700 px-3 py-1.5 rounded-lg">Upgrade</button>
                    <button className="text-xs bg-red-50 border border-red-200 text-red-600 px-3 py-1.5 rounded-lg">Revoke</button>
                  </div>
                </td>
              </tr>
            ))}
...
export default function PrimePage() {
  return (
    <LoginGate role="Prime Tailors Manager" department="Support Console — Panel 3" icon="⭐" accent="#d97706" accentLight="#fffbeb" storageKey="prime-manager">
      <PrimeContent />
    </LoginGate>
  );
}

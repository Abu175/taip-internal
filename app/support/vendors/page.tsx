...
export default function VendorsPage() {
  return (
    <SupportLayout title="Vendor Management" subtitle="Panel 5 — Manage all vendors, catalogs & payments">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <input type="text" placeholder="🔍  Search vendors..." className="border border-gray-200 rounded-lg px-4 py-2 text-sm text-gray-700 placeholder-gray-400 w-64 focus:outline-none focus:border-green-400" />
          ...
          <button className="bg-green-600 text-white text-sm px-4 py-2 rounded-lg hover:bg-green-700 transition-colors">+ Add Vendor</button>
        </div>
          ...
          <tbody>
            {vendors.map((v) => (
              <tr key={v.id} className="border-t border-gray-50 hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-4 text-sm font-semibold text-gray-800">{v.products}</td>
                <td className="px-4 py-4">
                  {v.rating ? <span className="text-sm font-semibold text-gray-800">★ {v.rating}</span> : <span className="text-gray-300">—</span>}
                </td>
                ...
                <td className="px-4 py-4">
                  <div className="flex gap-2">
                    ...
                    <button className="text-xs border border-gray-200 px-3 py-1.5 rounded-lg hover:bg-gray-50 text-gray-600">Catalog</button>
                    {v.status === "Pending" && (
                      <button className="text-xs bg-green-50 border border-green-200 text-green-700 px-3 py-1.5 rounded-lg hover:bg-green-100">Approve</button>
                    )}
                  </div>
                </td>
              </tr>

    ...
    <TechLayout title="Backend & APIs" subtitle="Panel 1 — Monitor microservices, APIs & system health">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <button className="text-xs bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors">🔄 Refresh</button>
        </div>
        <table className="w-full">
          <thead>
            <tr className="text-[11px] text-gray-400 uppercase tracking-wider bg-gray-50 border-b border-gray-100">
              ...
              <th className="text-left px-4 py-3">Requests/hr</th>
          ...
          <tbody>
            {services.map((s) => (
              <tr key={s.name} className="border-t border-gray-50 hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-4 text-sm text-gray-700 font-medium">{s.uptime}</td>
                <td className="px-4 py-4 text-sm text-gray-700">{s.latency}</td>
                <td className="px-4 py-4 text-sm text-gray-700">{s.requests}</td>
                <td className="px-4 py-4">
                  <div className="flex gap-2">
                    <button className="text-xs border border-gray-200 px-3 py-1.5 rounded-lg hover:bg-gray-50 text-gray-600">Logs</button>
                    <button className="text-xs bg-blue-50 border border-blue-200 text-blue-600 px-3 py-1.5 rounded-lg hover:bg-blue-100">Restart</button>
                  </div>
                </td>
              </tr>
            ))}
